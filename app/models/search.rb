class Search < ActiveRecord::Base
  ...
  def self.log(params, user)
    ...
    @search.search_params << SearchParam.create(:name=> 'price', :value_str => params[:currency_code], :value_int => params[:price].to_i) unless params[:price].blank? || params[:currency_code].blank?
    @search.save
  end
